...
export default function AddDeviceScreen() {
  ...
  const capturarFoto = async () => {
    ...
    if (status !== 'granted') return Alert.alert('Permissão', 'Precisamos de acesso à câmera para registrar o aparelho.');
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      if (user) {
        ...
        Alert.alert('Sucesso 🎉', `${nome} cadastrado! +50 pontos`);
  ...
  const excluirAparelho = (id: string) => {
    Alert.alert('Excluir', 'Deseja remover este aparelho?', [
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <TouchableOpacity style={styles.botaoFoto} onPress={capturarFoto}>
          {imagemUri ? <Image source={{ uri: imagemUri }} style={styles.imagemPreview} /> : <View style={{alignItems: 'center'}}><Ionicons name="camera" size={36} color="#4CAF50" /><Text style={{color: '#888', marginTop: 5}}>Tirar Foto do Aparelho</Text></View>}
        </TouchableOpacity>

        <TextInput style={styles.input} placeholder="Nome do Aparelho (ex: Geladeira)" placeholderTextColor="#888" value={nome} onChangeText={setNome} />
        <TextInput style={styles.input} placeholder="Potência em Watts (W)" placeholderTextColor="#888" keyboardType="numeric" value={potencia} onChangeText={setPotencia} />
        <TextInput style={styles.input} placeholder="Horas de uso por dia" placeholderTextColor="#888" keyboardType="numeric" value={horasPorDia} onChangeText={setHorasPorDia} />
...
const styles = StyleSheet.create({
  ...
  titulo: { color: '#fff', fontSize: 24, fontWeight: 'bold', textAlign: 'center', marginBottom: 20, marginTop: 20 },
  botaoFoto: { height: 140, backgroundColor: '#1E1E1E', borderRadius: 10, justifyContent: 'center', alignItems: 'center', marginBottom: 15, borderWidth: 1, borderColor: '#333', overflow: 'hidden' },
  ...
  card: { flexDirection: 'row', backgroundColor: '#1E1E1E', padding: 12, borderRadius: 10, marginBottom: 10, alignItems: 'center', borderWidth: 1, borderColor: '#2c2c2c' },
  imagemCard: { width: 50, height: 50, borderRadius: 8, marginRight: 12 },
  cardConteudo: { flex: 1 },
